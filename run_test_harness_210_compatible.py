...
It runs the combined ALL_210_CASES reference suite and retains the useful
cross-case monotonicity, scenario-ordering, and Monte Carlo sensitivity checks.

Required repository files:
    harness_core.py
    test_cases.py   # must define ALL_COMBINED_CASES (ALL_100_CASES alias also supported)
...
from test_cases import ALL_COMBINED_CASES
...
def actual_for(case: dict[str, Any]):
    ...
    if op == "composite_both":
        return {
            "additive": composite_additive(inp["domain_scores"], inp["betas"]),
            "multiplicative": composite_multiplicative_zero_collapse(
                inp["domain_scores"], inp["betas"]
            ),
        }
...
def close_enough(expected, actual, tol):
    ...
    if isinstance(expected, dict):
        if not isinstance(actual, dict) or set(expected) != set(actual):
            return False
        return max(abs(float(expected[k]) - float(actual[k])) for k in expected) <= tol
...
def difference(expected, actual):
    ...
    if isinstance(expected, dict):
        if not isinstance(actual, dict) or set(expected) != set(actual):
            return np.nan
        return max(abs(float(expected[k]) - float(actual[k])) for k in expected)
    if isinstance(expected, (list, tuple, np.ndarray)):
        if len(expected) != len(actual):
            return np.nan
...
def run_all() -> pd.DataFrame:
    """Run all combined reference cases and return case-by-case results."""
    rows = []
    for c in ALL_COMBINED_CASES:
        actual = actual_for(c)
